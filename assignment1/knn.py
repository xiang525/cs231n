...
for k in k_choices:
    ...
    # run knn for num_folds times
    for i in range(num_folds):
        X_train_list = []
        y_train_list = []
        # the ith list in X_train_folds stores validation data
        # others are training data
        for j in range(num_folds):
            if i!=j:
                X_train_list.extend(X_train_folds[i])
                y_train_list.extend(y_train_folds[i])
        ...
        # training
        knnclf= classifier = KNearestNeighbor()
        knnclf.train(X_train_data, y_train_data)
        
        # predicting
        dists = knnclf.compute_distances_no_loops(X_test)
